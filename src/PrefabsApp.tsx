...
import { type LoadedPrefab, PrefabService } from 'PrefabService';
...
export function PrefabsApp({ mouse }: { mouse: Mouse }) {
	...
	useEffect(() => {
		...
		const unsubscribe = PrefabService.onPrefabsChanged((updated) => setPrefabs(updated));
	...
	const handleCreatePrefab = async (selectedModel: Model, prefabName: string) => {
		...
		if (infoModule) setIsCreatingPrefab(false);
	};
	...
	return (
		...
		>
			{isPlacing && selectedPrefab ? (
				<PlacementUI prefab={selectedPrefab} onCancel={handleCancelPlacement} mouse={mouse} />
			) : (
				<PrefabLibrary
					mouse={mouse}
					prefabs={prefabs}
